...
use tarpc::transport::channel::{map_transport_to_server};
use tarpc::{
...
async fn main() -> anyhow::Result<()> {
    ...
    listener
        ...
        .max_channels_per_key(1, |t| t.transport().get_ref().get_ref().get_ref().peer_addr().unwrap().ip())
        // serve is generated by the service attribute. It takes as input any type implementing
        // the generated World trait.
        .map(|channel| {
            let server = HelloServer(channel.transport().get_ref().get_ref().get_ref().peer_addr().unwrap());
