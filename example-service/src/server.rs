...
async fn run(server_addr: SocketAddr) -> io::Result<()> {
    ...
    bincode_transport::listen(&server_addr)?
        ...
        .limit_connections(
            *server::Limits::default().channels_per_key(1).channels(10),
            |t| t.as_ref().peer_addr().unwrap().ip(),
        )
        // serve is generated by the service! macro. It takes as input any type implementing
        // the generated Service trait.
        .map_ok(|channel| {
            let server = HelloServer(channel.as_ref().peer_addr().unwrap());
            let handler = channel.respond_with(service::serve(server));
