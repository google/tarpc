...
fn bench_small_packet() -> io::Result<()> {
    ...
    let server = Server::<u32, u32>::new(server::Config::default())
        .incoming(
            listener
                .incoming()
                .compat()
    ...
    let bench = async move {
        ...
        let codec = bincode_transport::new(stream);
        let client = &mut await!(Client::<u32, u32>::new(client::Config::default(), codec));
