...
#[derive(Debug)]
pub struct InFlightRequests<Resp, SharedCtx> {
    request_data: FnvHashMap<u64, RequestData<Resp, SharedCtx>>,
    deadlines: DelayQueue<u64>,
}

impl<Resp, SharedCtx> Default for InFlightRequests<Resp, SharedCtx> {
    fn default() -> Self {
        Self {
...
#[derive(Debug)]
struct RequestData<Res, SharedCtx> {
    ctx: trace::Context,
    span: Span,
    response_completion: oneshot::Sender<Result<(SharedCtx, Res), RpcError>>,
...
impl<Res, SharedCtx> InFlightRequests<Res, SharedCtx> {
    ...
    pub fn complete_request(
        &mut self,
        request_id: u64,
        result: Result<(SharedCtx, Res), RpcError>,
    ) -> Option<Span> {
    ...
    pub fn complete_all_requests<'a>(
        &'a mut self,
        mut result: impl FnMut() -> Result<(SharedCtx, Res), RpcError> + 'a,
    ...
    pub fn poll_expired(
        &mut self,
        cx: &mut Context,
        expired_error: impl Fn() -> Result<(SharedCtx, Res), RpcError>,
    ) -> Poll<Option<u64>> {
