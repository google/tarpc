...
use tarpc::{
    ClientMessage, Request, Response, ServerError, Transport, client,
    server::{self, Channel},
    trace,
};
...
impl ExtractContext<CustomSharedContext> for ServerContext {
    fn extract(&self) -> CustomSharedContext {
        CustomSharedContext {
...
impl AfterRequest<ServerContext, WorldResponse> for SessionHook {
    async fn after(
        &mut self,
        ctx: &mut ServerContext,
        resp: &mut Result<WorldResponse, ServerError>,
    ) {
        if resp.is_ok()
            && let Some(id) = ctx.session_id
        {
...
async fn main() -> anyhow::Result<()> {
    ...
    let client_transport =
        client_transport.with(|f: ClientMessage<ClientContext, WorldRequest>| {
            async move {
                ...
                }) = f
                {
                    ...
                    tokio::time::sleep(Duration::from_secs(
                        context.delay_sending_by_seconds as u64,
                    ))
                    .await;
                }
    ...
    tokio::spawn(
        server
            .execute(HelloServer.serve().before_and_after(hook))
            .for_each(spawn),
    );
    ...
    let hello = client
        .hello(&mut client_context, "Stan".to_string())
        .await?;
    ...
    let hello = client
        .hello(&mut client_context, "Frank".to_string())
        .await?;
    assert_eq!(hello, Ok("Hello, Frank!".to_string()));

    let hello = client
        .hello(&mut client_context, "Joshua".to_string())
        .await?;
    assert_eq!(hello, Err("Give me more money".to_string()));
...
fn map_request_context<Ctx, Ctx2, Req>(
    req: ClientMessage<Ctx, Req>,
    f: impl FnOnce(Ctx) -> Ctx2,
) -> ClientMessage<Ctx2, Req> {
    match req {
...
fn map_response_context<Ctx, Ctx2, Res>(
    res: Response<Ctx, Res>,
    f: impl FnOnce(Ctx) -> Ctx2,
) -> Response<Ctx2, Res> {
    Response {
...
fn create_channel() -> (
    impl Transport<ClientMessage<ClientContext, WorldRequest>, Response<ClientContext, WorldResponse>>,
    impl Transport<Response<ServerContext, WorldResponse>, ClientMessage<ServerContext, WorldRequest>>,
) {
    let (client, server): (
        UnboundedChannel<
            Response<CustomSharedContext, WorldResponse>,
            ClientMessage<CustomSharedContext, WorldRequest>,
        >,
        UnboundedChannel<
            ClientMessage<CustomSharedContext, WorldRequest>,
            Response<CustomSharedContext, WorldResponse>,
    ...
    let client = client
        .with(|m| futures::future::ok(map_request_context(m, |c: ClientContext| c.extract())))
        .map_ok(|r| {
            map_response_context(r, |c: CustomSharedContext| ClientContext {
                session_id: c.session_id,
                delay_sending_by_seconds: 0,
            })
        });
    let server = server
        .with(|r| futures::future::ok(map_response_context(r, |c: ServerContext| c.extract())))
        .map_ok(|m| {
            map_request_context(m, |c| ServerContext {
                deadline: c.deadline,
                trace_context: c.trace_context,
                session_id: c.session_id,
                balance: 0,
            })
        });
