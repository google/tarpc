...
#![plugin(serde_macros)]
#![feature(custom_derive, plugin, default_type_parameter_fallback, test, inclusive_range_syntax)]
...
use tarpc::{Await, Connect, RpcResult};
...
fn run_once(clients: &[FutureClient], concurrency: u32, print: bool) {
    ...
    let sum_latencies = futures.into_iter()
        .map(|future| {
            let start = future.await().unwrap().1.to_duration();
...
fn main() {
    ...
    let clients: Vec<_> = (1...5).map(|_| {
        FutureClient::connect(server.local_addr()).unwrap()
    }).collect();
